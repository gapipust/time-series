...
```{r fig.cap="Daily Page Loads", out.width='75%', out.height='75%', fig.align='center'}
...
```{r fig.cap="Daily Page Loads - training set", out.width='75%', out.height='75%', fig.align='center'}
...
```{r message=FALSE, warning=FALSE, fig.cap="Histogram of Daily Page Loads", fig.align='center', out.width='75%', out.height='75%'}
...
In the figure below we presented boxplots of Page Loads by day of the week.

```{r message=FALSE, warning=FALSE, fig.cap="Boxplot of Page Loads by day of the week", fig.align='center', out.width='75%', out.height='75%'}
...
```{r fig.cap="Correlation lag plots", fig.align='center', out.width='75%', out.height='75%'}
...
```{r message=FALSE, warning=FALSE, fig.cap="Plots of differenced data", fig.align='center', out.width='75%', out.height='75%'}
...
Seasonally (with period of 7 days) and regularly differenced data, $\Delta_{7}\Delta Page Loads train$ seems more stationary. This implies a unit root $d=1$ as well as a seasonal unit root, $D=1.$ We can see that ACF decays to zero quicker than PACF indicating strong MA component of the model. ACF shows significant correlation at lags 7 and 14, which implies $q=3$ and $Q=2$. PACF shows significant correlation at lags 7, 14, 21, 28, 35, 42 and 49, which suggests $p=7$. 
...
```{r fig.align='center', fig.cap="Weekly Page Loads", message=FALSE, warning=FALSE, out.width='75%', out.height='75%'}
...
```{r fig.cap="Weekly Page Loads - training set", fig.align='center', out.width='75%', out.height='75%'}
# Training set: First 4.5 years, Test set: Last 6 months
cutoff_date <- as.Date("2020-02-19")
...
```{r message=FALSE, warning=FALSE, fig.align='center', fig.cap="Plots of differenced data", out.width='75%', out.height='75%'}
...
```{r fig.cap="Monthly Average Page Loads", fig.align='center', out.width='75%', out.height='75%'}
# Monthly Aggregation
ts_website_monthly <- apply.monthly(ts_website, mean)
plot(ts_website_monthly, main = "Monthly Average Page Loads", 
     ylab = "Page Loads", xlab = "Date")
```
We divide the data into a training set and a test set. The test set will contain the last 6 months of observations.
...
plot(train_data_monthly, main = "Weekly Page Loads", 
     ylab = "Page Loads", xlab = "Date")
```
...
Similar to the process before we try to find the best model for the weekly data. Here the seasonality seems yearly (period of 12 months). We try with the model $SARIMA(0,1,0)\times(0,1,1)_{12}$.

```{r message=FALSE, warning=FALSE, fig.align='center', fig.cap="Plots of differenced data", out.width='75%', out.height='75%'}
dlm12.1 <- diff(diff(train_data_weekly,12),1)
plot(dlm12.1,lwd=1,main=expression(paste(Delta,Delta[12], "Monthly Page Loads train")))
invisible(acf2(dlm12.1,main=expression(paste(Delta,Delta[12], "Monthly Page Loads train"))))
```

```{r message=FALSE, warning=FALSE, results='hide', fig.align='center', out.width='75%', out.height='75%'}
monthly_visitors.fit1=sarima(train_data_monthly,0,1,0,0,1,1,12)
```

```{r message=FALSE, warning=FALSE, fig.align='center', out.width='75%', out.height='75%'}
monthly_visitors.fit1
```
